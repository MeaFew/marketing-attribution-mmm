...
| 参照系 | R² | Adj. R² | 说明 |
|--------|-----|---------|------|
| **MMM 领域基准（Ridge）** | 0.70–0.85 | — | 多营销渠道回归的典型范围（参考文献：Hanssens et al., *Market Response Models*, 2nd ed.） |
| ** naive 均值预测** | ~0.35 | — | 用历史 revenue 均值作为预测 |
| **单变量（最大渠道）** | ~0.55 | — | 仅用 spend 最高的单一渠道回归 |
| **本项目 OLS** | ~0.85 | 0.82 | 全渠道线性回归，VIF < 5 |
| **本项目 Ridge** | **~0.85** | **0.82** | L2 正则化，进一步稳定系数估计 |
| **本项目 Lasso** | ~0.84 | 0.81 | L1 正则化，自动特征选择 |
...
|------|-----|---------|---------------|-----------|
| OLS | ~0.85 | 0.82 | — | < 5 |
| **Ridge** | **~0.85** | **0.82** | α = 1.0 | < 5 |
| Lasso | ~0.84 | 0.81 | α = 0.1 | < 5 |

> **选择 Ridge 作为最终模型**：在保持可解释性的同时，通过 L2 正则化消除 Google Paid Search 与 Meta Facebook 之间的多重共线性，所有 VIF < 5。

**Durbin-Watson ≈ 2.0**，残差无显著自相关，模型满足经典线性回归假设。
